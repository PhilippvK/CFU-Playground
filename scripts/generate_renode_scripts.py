...
sys.path.append((os.path.join(os.path.dirname(__file__), "..", "third_party", "python", "litex", "litex", "tools")))

litex_renode = import_module("litex_json2renode")


def generate_resc(target, cfu_lib_filepath, gdbserver_port=10001, enable_analyzer=True):
    result = (
        """
using sysbus
mach create \""""
...
"""
    ...
    if gdbserver_port is not None or gdbserver_port == 0:
        result += "machine StartGdbServer 10001\n"
    if enable_analyzer:
        result += """showAnalyzer sysbus.uart
...
def parse_args():
    ...
    parser.add_argument("conf_file", help="JSON configuration generated by LiteX")
    ...
    parser.add_argument("--gdbserver-port", type=int, default=10001, help="Port to use for GDBServer (0: disabled)")
    parser.add_argument(
        "--disable-analyzer", dest="enable_analyzer", action="store_false", help="Disable UART Analyzers"
    )
    parser.add_argument(
        "--etherbone",
        action="append",
...
def main():
    ...
    if os.path.isfile(predefined_resc_path):
        ...
        with open(resc_filepath, "w") as f:
            ...
            f.write(
                generate_resc(
                    args.target, cfu_lib_filepath, gdbserver_port=gdbserver_port, enable_analyzer=args.enable_analyzer
                )
            )
