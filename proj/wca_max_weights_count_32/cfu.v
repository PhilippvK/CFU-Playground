module Cfu (
    ...
    reg signed [7:0] active_clusters [0:MAX_WEIGHTS_CNT];
    integer i, j;
    ...
    always @(posedge clk) begin
        if (reset) begin
            ...
            for (i = 0; i < MAX_WEIGHTS_CNT; i = i + 1) active_clusters[i] <= 0;
            ...
            if (cmd_valid && !rsp_valid) begin
                ...
                if (funct7 == CFU_FUNCT7_SET_CODEBOOK_2) begin
                    ...
                    if (last_cluster_num == 2) begin
                        for (i = 0; i < (MAX_WEIGHTS_CNT < 64 ? MAX_WEIGHTS_CNT : 64); i = i + 1) begin
                            if (i < 32) begin
                                active_clusters[i] <= clusters_2[(cmd_payload_inputs_0 >> i) & 1];
                            end
                            else if (i < 64) begin
                                active_clusters[i] <= clusters_2[(cmd_payload_inputs_1 >> i) & 1];
                            end
                        end
                    end
                    else if (last_cluster_num == 4) begin
                        for (i = 0; i < (MAX_WEIGHTS_CNT < 32 ? MAX_WEIGHTS_CNT : 32); i = i + 1) begin
                            if (i < 16) begin
                                active_clusters[i] <= clusters_4[cmd_payload_inputs_0[(2*i)+1 -: 2]];
                            end
                            else if (i < 32) begin
                                active_clusters[i] <= clusters_4[cmd_payload_inputs_1[(2*(i-16))+1 -: 2]];
                            end
                        end
                    end
                    else if (last_cluster_num == 16) begin
                        for (i = 0; i < (MAX_WEIGHTS_CNT < 16 ? MAX_WEIGHTS_CNT : 16); i = i + 1) begin
                            if (i < 8) begin
                                active_clusters[i] <= clusters_16[cmd_payload_inputs_0[i*4 +: 4]];
                            end
                            else if (i < 16) begin
                                active_clusters[i] <= clusters_16[cmd_payload_inputs_1[(i-8)*4 +: 4]];
                            end
                        end
                    end
