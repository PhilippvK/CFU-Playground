module Cfu (
    ...
    parameter PARALLEL_MULS = 4; // 1, 2, 4 or 8
    parameter MUL_STAGES = 8 / PARALLEL_MULS;
    reg [2:0] mac_step;   // up to 8 steps
    reg busy;
    reg signed [31:0] acc;
        ...
        if (reset) begin
            ...
            if (cmd_valid && !rsp_valid && !busy && funct7 == CFU_FUNCT7_ALU_MAC) begin
                ...
                for (j = 0; j < PARALLEL_MULS; j = j+1) begin
                    index = mac_step * PARALLEL_MULS + j;
                    if (index < 8) begin
                        prod[index] = ws[index] * (acts[index] + 128);
                        sum[index] <= sum[index] + prod[index];
                    end
                end

                if ((mac_step + 1) >= MUL_STAGES) begin
                    // DONE
                    // TODO: decoupled?
                    rsp_payload_outputs_0 <= 32'hABCD0001;
                    rsp_valid <= 1;
                ...
                */
                   ...
                   rsp_payload_outputs_0 <= sum[0] + sum[1] + sum[2] + sum[3] +
